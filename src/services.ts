...
const serviceClearSingle = function (options: any = {}) {
...
const serviceClearGroup = function (options: any = {}) {
  const redisClient = options.redisClient || DEFAULT_REDIS_CLIENT;
...
const serviceClearAll = function (options: any = {}) {
  const redisClient = options.redisClient || DEFAULT_REDIS_CLIENT;
...
export default (options: any = {}) => {
  ...
  return function () {
    ...
    if (!DISABLE_REDIS_CACHE) {
      app.use(`${pathPrefix}/clear/single`, serviceClearSingle({ redisClient }));
      app.use(`${pathPrefix}/clear/group`, serviceClearGroup({ redisClient }));
      app.use(`${pathPrefix}/clear/all`, serviceClearAll({ redisClient }));
    }
  };
}
